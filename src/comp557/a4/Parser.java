...
public class Parser {
	...
	public static Material createMaterial(Node dataNode) {
		...
		if (refAttr != null) {
			...
			Node refract = dataNode.getAttributes().getNamedItem("refract");
			if (refract != null) {
				if (refract.getNodeValue().equalsIgnoreCase("true")) {
			...
			Node index = dataNode.getAttributes().getNamedItem("index");
			if (index != null) {
				material.index = Double.parseDouble(index.getNodeValue());
			}
	...
	public static Render createRender(Node dataNode) {
		...
		Node reflect = dataNode.getAttributes().getNamedItem("reflect");
		if (reflect != null) {
			if (reflect.getNodeValue().equals("true")) {
				render.reflect = true;
			}
		} else {
			render.reflect = false;
		}
		...
		Node jitter = dataNode.getAttributes().getNamedItem("jitter");
		if (jitter != null) {
			if (jitter.getNodeValue().equalsIgnoreCase("true")) {
				render.jitter = true;
				System.out.println("jitter = true");
			}
		} else {
			render.jitter = false;
			System.out.println("jitter = false");
		}
